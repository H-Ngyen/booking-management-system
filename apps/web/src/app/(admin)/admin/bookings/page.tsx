...
export default function AdminBookingsPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-ink">Quản lý booking</h1>
        <p className="text-sm text-ink-3">Xem, xác nhận, hoàn thành hoặc hủy toàn bộ booking.</p>
      </div>
      ...
      {data && data.items.length === 0 && <EmptyState title="Không có booking nào" />}
      ...
      <Dialog open={cancelId != null} onOpenChange={(open) => !open && setCancelId(null)}>
        <DialogContent>
          ...
          <form onSubmit={handleCancel} className="space-y-4" noValidate>
            <Field label="Lý do hủy" htmlFor="admin-cancel-reason" error={reasonError}>
              <Textarea
